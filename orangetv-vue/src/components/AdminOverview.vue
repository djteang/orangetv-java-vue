...
import { Activity, ArrowUpRight, RefreshCw, Search, Users, Video } from 'lucide-vue-next'
...
const metrics = computed(() => [
  ...
  { label: '搜索记录', value: props.stats?.totalSearches, icon: Search, detail: '当前保存的关键词记录', note: props.stats ? `今日更新 ${number(props.stats.todaySearches)} 条` : '今日更新', featured: false },
...
<template>
  <section class="admin-overview" aria-label="管理面板数据概览" :aria-busy="loading">
    ...
    <div class="overview-metrics">
      <article v-for="metric in metrics" :key="metric.label" class="metric-card" :class="{ 'metric-card--featured': metric.featured }">
        ...
        <div class="metric-bottom"><span>{{ metric.detail }}</span><ArrowUpRight v-if="metric.featured" :size="15" aria-hidden="true" /></div>
